...
private:
    ...
    static void activityTimerCallback(void* arg) {
        ...
        if (self->idle_seconds_ >= DEEP_SLEEP_SECONDS) {
            ESP_LOGI(TAG, "Deep sleep after %ds idle", DEEP_SLEEP_SECONDS);
    ...
    void InitializeDisplay() {
        ...
        // Fix display orientation: 180° rotation via raw I2C commands
        // Must be BEFORE OledDisplay constructor (raw I2C after it crashes on OTA)
        // SH1106: 0xA1 = segment remap reverse, 0xC8 = COM scan reverse
        {
            uint8_t cmd_seg[] = {0xA1};  // Segment remap: reverse (flip horizontal)
            esp_lcd_panel_io_tx_param(panel_io_, 0x00, cmd_seg, 1);
            uint8_t cmd_com[] = {0xC8};  // COM scan direction: reverse (flip vertical)
            esp_lcd_panel_io_tx_param(panel_io_, 0x00, cmd_com, 1);
            ESP_LOGI(TAG, "Display orientation fixed (180° rotation)");
        }
